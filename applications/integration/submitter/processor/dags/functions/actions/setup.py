...
from functions.interface.linux import linux_open_directory, linux_format_list
from functions.interface.lmod import lmod_module_command
from functions.interface.venv import venv_activate_command, venv_create_command, venv_check_creation, venv_format_packages, venv_check_packages, venv_install_command, venv_check_installation
from functions.interface.sftp import stfp_store_file

from functions.storage.management import object_storage_interaction
from functions.utility.files import files_write_data, files_chmod_command
...
def setup_venv_create(
    ...
    create_command = venv_create_command(
        name = venv_name
    )

    activation_command = venv_activate_command(
        path = '',
        name = venv_name
    )
    ...
    venv_exists = venv_check_creation(
        resulted_print = interaction_output
    )
...
def setup_venv_packages(
    ...
    path_activation_command = venv_activate_command(
        path = venv_path,
        name = venv_name
    )
    ...
    package_list = venv_format_packages(
        resulted_print = interaction_output
    )
    
    missing_packages = venv_check_packages(
...
def setup_venv_install(
    ...
    package_command = venv_install_command(
        wanted_packages = venv_packages
    )
    ...
    venv_installed = venv_check_installation(
        resulted_print = interaction_output
    )
...
def setup_send_file(
    ...
    if source_place_split[0] == 'storage':
        if source_place_split[1] == 'allas':
            ...
            local_file_path = files_write_data(
                name = source_path_split[-1],
                data = file_data
            )
    ...
    if not local_file_path is None:
        ...
        send_result = platform_interface_interaction(
            storage_parameters = storage_parameters,
            lock_location = lock_location,
            interaction_parameters = {
                'platform': target_platform,
        ...
        if remote_file_name in file_list:
            ...
            if '.pem' in remote_file_name:
                chmod_command = files_chmod_command(
                    file_path = remote_file_path
                )
                # Could be worthwhile to check if a file has permissions
                chmod_result = platform_interface_interaction(
                    storage_parameters = storage_parameters,
                    lock_location = lock_location,
                    interaction_parameters = {
                        'platform': target_platform,
